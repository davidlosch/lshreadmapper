...
float FilterValue (const float *filterWeights,
    const int &x, const int &y)
{
    return filterWeights[(x+FILTER_SIZE) + (y+FILTER_SIZE)*(FILTER_SIZE*2 + 1)];
}

void performOnCPU(const Image &image, float *filter, Image &dstImage)
{
    // Perfom the filter on the CPU (no multi-threading)

    std::chrono::high_resolution_clock::time_point timeStartCPU = std::chrono::high_resolution_clock::now();

    for (int row = 0; row < image.height; row++)
    {
        for (int column = 0; column < image.width; column++)
        {
            // Sum of the weighted neighbour pixel intensities for the current pixel
            ...
            for (int y = -FILTER_SIZE; y <= FILTER_SIZE; y++)
            {
                ...
                {
                    int imageCoordY = (row + y), imageCoordX = (column + x);
                    // Range check for coordinates
                    if (imageCoordY >= 0 && imageCoordY + 2 < image.height && imageCoordX >= 0 && imageCoordX + 2 < image.width)
                    {
                        float currentFilter = FilterValue(filter, x, y);

                        // Each pixel consists of 4 elements (alpha is ignored)
                        sumR +=currentFilter
                            * (unsigned char)image.pixel.at(imageCoordY * (image.width * 4) + (imageCoordX * 4));
                        sumG +=currentFilter
                            * (unsigned char)image.pixel.at((imageCoordY * (image.width * 4) + (imageCoordX * 4)) + 1);
                        sumB +=currentFilter
                            * (unsigned char)image.pixel.at((imageCoordY * (image.width * 4) + (imageCoordX * 4)) + 2);
                    }
                }
            }

            dstImage.pixel.at( row * (image.width * 4) + (column * 4)) 		= (int)sumR ;
            dstImage.pixel.at((row * (image.width * 4) + (column * 4)) + 1) = (int)sumG ;
            dstImage.pixel.at((row * (image.width * 4) + (column * 4)) + 2) = (int)sumB ;
        }
    }

    std::chrono::high_resolution_clock::time_point timeEndCPU = std::chrono::high_resolution_clock::now();
    std::chrono::microseconds durationCPU = std::chrono::duration_cast<std::chrono::microseconds>(timeEndCPU - timeStartCPU);
    std::cout << "Duration CPU: " << durationCPU.count() << std::endl;

    SaveImage (RGBAtoRGB (dstImage), "outputCPU.ppm");
}
...
int main ()
{
    ...
    // dstImage for CPU; copied from original to preserve dimension
    Image dstImage = image;

    // //////////////////////////////////////////////////////////////

    performOnCPU(image, filter, dstImage);
    ...
    // Third argument 0, so this command is executed in-order
    ...
    std::chrono::microseconds duration = std::chrono::duration_cast<std::chrono::microseconds>(timeEnd - timeStart);
    std::cout << "Duration GPU: " << duration.count() << std::endl;
